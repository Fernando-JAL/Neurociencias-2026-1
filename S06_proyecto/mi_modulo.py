...
def grafo3D(coords, Hub, pos):
    x, y, z = coords['x'].values, coords['y'].values, coords['z'].values #Obtener coordenadas
    
    nodes_size = [30 if idx != Hub[0] else 200 for idx in coords.index] #Elegir tamaño de los nodos i el nodo NO es el hub → tamaño 30 si es 200 es el hub
    pos.scatter(x, y, z, alpha=0.5, s=nodes_size) #Dibujar los nodos en 3D
    for idx, (x_, y_, z_) in enumerate(zip(x, y, z)):
        pos.text(x_, y_, z_, coords.index[idx], fontsize=5)
        if coords.index[idx] == Hub[0]:  #Volver a marcar el HUB en rojo y grande
            pos.text(x_, y_, z_, 'HUB', color='red', fontweight='bold', fontsize=10)

def grafo_comunidades(comunidades, Hub, coords, pos):
    x, y, z = coords['x'].values, coords['y'].values, coords['z'].values #Extrae coordenadas 3D
    
    nodes_size = [30 if idx != Hub[0] else 200 for idx in coords.index] #Hace lista de tamaños (hub grande, demás pequeños)
    pos.scatter(x, y, z, alpha=0.5, s=nodes_size) #Grafica todos los puntos 3D,,, pos es el eje 3D donde se dibuja.
    for idx, (x_, y_, z_) in enumerate(zip(x, y, z)):
        pos.text(x_, y_, z_, coords.index[idx], fontsize=5)
        if coords.index[idx] == Hub[0]:
            pos.text(x_, y_, z_, 'HUB', color='red', fontweight='bold', fontsize=10)
    
    colores = ['red', 'green', 'blue', 'black', 'orange', 'yellow']  #Conectar nodos dentro de cada comunidad
    for n_comunidad, comunidad in enumerate(comunidades):
        for idx in range(len(comunidad)-1):
            ...
            x_ = [coords.loc[n1, 'x'], coords.loc[n2, 'x']] #Obtiene las coordenadas de cada par de nodos
            y_ = [coords.loc[n1, 'y'], coords.loc[n2, 'y']]
            z_ = [coords.loc[n1, 'z'], coords.loc[n2, 'z']]
            pos.plot(x_, y_, z_, linewidth=3, alpha=0.4, color=colores[n_comunidad]) #Dibuja la línea del color de esa comunidad
