...
def estadisticas_ensayo(th25, th50, th75, promedio, mu_sigma):
    ...
    for idx, columna in enumerate(estadisticos_df.columns):
        d1 = np.min(columnas[idx])
        d2 = np.max(columnas[idx])
        d3 = np.median(columnas[idx])
        d4 = stats.median_abs_deviation(columnas[idx])
