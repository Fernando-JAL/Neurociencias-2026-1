...
def grafo_comunidades(comunidades, Hub, coords, pos, titulo='Comunidades'):
    x, y, z = coords['x'].values, coords['y'].values, coords['z'].values
    ...
    if len(comunidades) > len(colores):
        colores = colores * (len(comunidades) // len(colores) + 1)
